...
| `--new` | 在终端里建一份空白部署再启动它:选代码包(仓内 `bots/` 与已装的 bot 类扩展)、起目录名、取个名字;bot 扩展声明了 png 图标时复制成部署的 `avatar.png` |
...
`pnpm start`、`start.bat` 与 `start.sh` 都调用 `bin/cortico.mjs`。它安装缺失的依赖、在控制台产物缺失、不完整或落后于源码时构建，
