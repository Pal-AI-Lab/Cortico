...
内置 World 和供应商也列入已安装，不能单独删除；World 状态取当前部署的实际挂载状态，供应商取该部署的启用策略。
...
包可在 `cortico.displayName` 声明显示名称，市场读取当前页包的元数据，无需下载或执行扩展。未声明时，已安装包可使用导出定义的名称，其余使用包名；包名另列于标题下。
...
新 World 和供应商扩展需要重启注册，新 Bot 模板可直接用于 `pnpm start --new` 创建实例。
...
供应商模块启用策略保存在部署的 `extension-state.json`；首次迁移启用已注册模块，此后新增外部模块默认禁用，内置模块默认启用且可单独停用。
停用模块保留共享连接配置，并阻止新绑定、预览、模型列表和运行时操作。当前主模型或绑定任务使用的模块不能停用。
