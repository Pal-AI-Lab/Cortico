...
配置组声明的每个路径都要落在自己的段内,否则是失败:写回按路径走,值会落到别人的段上。
World 是 `worlds.<id>.`,并且要在 `defaults()` 里有对应项——缺默认值时控制台照样渲染旋钮、
照样写回 config.json,而读到的是代码里另一处的兜底值。provider 是 `providers.<端点名>.options.`:
地址、密钥变量名、图像开关与 `spec` 归框架的连接配置组,模块只写自己的 `options`;
这些值存在端点条目里,不经 `defaults()`,因此只核对段。
