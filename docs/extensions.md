<!-- Owner: src/extensions.ts, src/extensions/manifest.ts, src/extensions/package-store.ts, src/extensions/activation.ts -->
...
控制台按 World、供应商、Bot 模板分类。每类包含已安装、扩展市场和手动安装。
已安装按内容宽度显示 1–3 列，默认每页两行；市场默认每页四行。每个分类分别保留筛选、每页数量和分页位置，用户可覆盖自动容量。
内置 World 和供应商也列入已安装，不能单独删除；World 状态取当前部署的实际挂载状态，供应商显示安装状态。
已安装卡片的作者与市场使用同一发布者信息；市场未返回此包时取本机 `package.json.author`，两者均缺失时显示“未提供”。标题取扩展定义的显示名称，无显示名称时使用包名。
内置扩展作者为 phantivia。已安装详情与市场共用简介、信息、历史版本和外部链接；本机包提供离线元数据，npm 包另取发布历史。内置扩展没有独立 npm 发布历史。
包可在 `cortico.displayName` 声明显示名称，市场读取当前页包的元数据，无需下载或执行扩展。未声明时，已安装包可使用导出定义的名称，其余使用包名；市场的包名与版本相邻显示，已安装包的包名放在详情中。
仓库、主页和问题反馈链接分别来自 `repository`、`homepage` 和 `bugs`；未提供时不显示对应入口。
手动安装明确选择 npm 包名（可带精确版本或标签）或运行 Cortico 的主机目录。目录通过 link 接入。

## 安装事务与更新

控制台在共享环境的同级隔离目录安装，使用子进程检查 manifest、入口、默认导出、声明及控制台资源，
不调用扩展的 start()。检查成功才切换正式目录；失败保留旧环境。跨进程锁串行化共享目录写入，
操作记录、依赖缓存和 journal 保存于同级 `.cortico-extensions-*` 目录，该目录被 Git 忽略。安装失败也保留记录，启动时恢复中断事务；恢复失败时拒绝继续覆盖。
同一个 operation ID 不重复安装，断线后可查询结果。隔离目录校验不是安全沙箱，扩展具有宿主用户权限。
扩展环境使用自己的 `extensions/pnpm-workspace.yaml` 审核依赖构建脚本。未审核的脚本会阻止安装；在该文件的 `allowBuilds` 中按包和版本批准后重试。安装事务复制此配置到隔离目录，不读取仓库根目录的构建许可。

更新检查比较磁盘版本与 npm latest。本机链接不查 npm；不兼容版本拒绝安装。运行版本与磁盘版本不同时显示待重启。
新 World 扩展需要重启注册，新供应商在安装提交后自动注册。共享环境中的其他运行实例在读取扩展或供应商列表时注册新增供应商。新 Bot 模板可直接用于 `pnpm start --new` 创建实例。
更新当前 Bot 正在使用的代码需重启。控制台按 deployment 身份、新 bootId 和 ready 状态恢复，120 秒后提供继续检测。

## 当前 Bot 的使用状态

World 的加载和卸载复用 WorldAssembly，保存 `worlds.<id>.enabled`，保留 Agent 可见性。
供应商安装后自动注册，不提供手动启停；已注册供应商的代码更新仍需重启。启动时从部署的 `extension-state.json` 移除旧 `providers` 启用策略和供应商待加载意图，保留 World 意图与诊断。旧策略中禁用的已安装供应商也会注册。
当前主模型、绑定任务或连接配置使用的供应商不能删除。删除未使用的供应商会移除其运行时注册与控制台资源。
Bot 模板只提供创建实例说明，不提供运行中的人格替换。

“重启后加载”保存绑定包名、版本和模块 ID 的一次意图；新进程注册后只执行一次，失败保留原因。
“彻底删除”修改共享安装环境，检查已知部署、共享供应商配置、依赖和运行实例登记。
运行实例使用独立随机登记文件及心跳，过期但 PID 仍存活时按无法确认处理；不能据此宣称已检查所有外部项目。
无法判定其他部署的 World 模板默认启用状态时拒绝删除，并指出需确认的部署。
删除包不删除部署配置、记忆、模型缓存或本地链接源码。已导入的删除记录保留到当前进程重启。

命令行直接执行 pnpm 绕过控制台事务与引用检查。手工操作共享环境前应停止使用该环境的进程，
并保留 package.json、lockfile 和模块目录的完整备份。扩展目录及其同级事务目录属于部署状态，不提交到版本控制。
